...
In [this video](https://www.youtube.com/watch?v=443UNeGrFoM) he explains, why and how he writes C.
...
Maybe because this heresy wasn't founded centralized by some specific person, but was formed independently by different people.
...
* C++ has destructors.
  ...
  Destructors allow to implement RAII semantics, which greatly simplifies code compared to manual calls to cleanup functions (memory free, file close, etc.) like in C code.
...
* C++ supports dynamic type-safe polymorphism via inheritance.
  ...
  C++ is often critisized, because it's possible to create huge inheritance hierarchies, but this criticism isn't correct - the language itself doesn't force one to create such hierarchies, one doesn't need to use them, if it isn't really necessary.
