...
I name this trend _pure C heresy_.
But why heresy?
...
Curiosly he reinvents a lot of stuff (like table-based code generation), because C is too weak to achive the same functionality via language means.


### Notable programming languages affected by pure C heresy

Pure C heresy affect not only C++.
...
[Go](https://go.dev/) programming language is also a good example of pure C heresy, but somewhat evolved into a much better, but still heretic language.
...
And it really adds many interesting imporvements and even has some sort of templates (parametric generics).
But it's still fundamentally flawed - it has no proper destructors, only `defer`.
Thus it's yet another form of pure C heresy.
...
* C++ has stronger type system.
  C++ doesn't allow by default conversion between unrelated pointer types, doesn't allow to violate mutability so easily.
...
* C++ has destructors.
  I found destructors to be the core feature of the whole language and overall one of the most significant programming inventions.
...
* C++ supports dynamic type-safe polymorphism via inheritance.
  ...
  C++ is often critisized, because it's possible to create huge inheritance hierarchies, but this criticism isn't correct - the language itself doesn't force one to create such hierarchies, one don't need to use them, if it isn't really necessary.
...
Excepthions are also one of the greatest C++ sins.
...
Also it doesn't contain many elements, which C++ inherited from C, which are considered to be harmful, at least in such unsafe form.

Ü has destructors, functions and operators overloading, type templates, function templates, namespaces - all neat additions of C++.
...
What in C++ is often a runtime crash (if one has a luck to catch such crash in debugging) is compilation error in Ü.
...
### Possible reasons for pure C heresy

But why exactly some people leave holy C++ chirch and fall into pure C heresy?
...
* Some beginner programmers may get scared of C++ complexity and they reject it, because they don't want to invest their time to learn C++ properly.
  For them it's much easier to continue write pure C (which is easier to learn) or use C++ in C style.
...
* Some programmers go further and learn C++, but eventually reject it, because of some scarry C++ things.
  ...
  Also back than OOP-style was heavily used (Java influence) and big inheritance hierarchies were notable scarry.
...
* Some game developers are also vulnerable to pure C heresy.
  Earlier games were mostly written in C, especially for consoles, sinse console developers provided only C compilers for their platforms.
  ...
  Even with C++ compilers availability some people preferred to use pure C, since some C++ features were to costly for games, especially features relying on heap allocation.
* A rare example of pure C heretics are programmers, who previosly used slow languages like Java or Python, but switched to C in order to achieve necessary for their tasks performance.
  They are usually assume that C is the best language to achieve great performance, but they don't necessary know, that C++ offers the same performance, but with better programming speed and less errors.
...
They often criticize RAII ideom and say, that managing a lot of small memory allocations via RAII, what is usual in C++, leads to slower running code.
...
The main advantage of C++ is that in places, where squeezing every bit of performance isn't necessary, one can use RAII ideom (like standard library containers), which is generally easier compared to managing each allocation individually, what is typical for C programs.
...
At the end typical C++ program may be better optimized compared to a pure C program, since more time budget is left for such optimizations.
...
And generally C++ code tends to compile-time polymorphism, compared to pure C, where runtime function pointers and `void*` pointers are still widely used for same things.
...
So it's easy to write errornous code, which compiles fine.
But it will obviosly not work as expected.
And how C heretics deal with it?
They debug their code to check if it's working, and if not (what is the most probable outcome), they need all power of modern debuggers to find, what is wrong.
And each even minor code change may require such tedious debugging.
So, if one needs to launch a debugger again and again, it's better to save at least some time not wayting too many for compilation to end.

At the and a typical C heretic may spend more time debugging his code compared to writing it.
Some expireneced pure C heretics even go further and write their own code for debugging simplification, like debug allocators, tracing code, etc.
And I admit, this may be sometimes useful.
But the whole approach is fundamentally flowed.
...
One need to inspect call stacks and variables to reason, what's goint wrong.
...
But with C it's hard to do so, since errornous code is inevitable in this language, one need to use something better - like C++.
C++ in many cases just doesn't allow to write code, which is obviously incorrect, it has stricter type system preventing many errors and has better abstractions (like templates and RAII), which eliminates typeical C mistakes.
...
* LLVM project.
  It includes C++ compiler frontend (*clang*), architecture-independnet optimisation pipelines, code generators for diverse set of CPUs, linker (*lld*), debugger, etc.
  Its main concurrent - GCC was first written in C, but was switched to C++.
* Chromium.
  This is one of two modern browsers, many others are just Chromium derivatives.
  It's so huge, that it's practicaly impossible nowdays to write a browser without using Chromium codebase, since it requires writing millions lines of code.
* Unreal engine.
  ...
  There are only a couple of concurrents, most of them are also being developing 15+ years.
  Writing something similar from scratch isn't viable.
* Microsoft Office.
  It's not open-source, but it's known from some information from its developers, that it's written in C++ and is huge.

Notable exceptions of large pure C codebases:

* Linux kernel is writen in C, since its development was started when C++ was still in its early days.
  Also Linus Torvalds actively opposes C++ usage.

I don't say thaat it's generally impossible to write a huge project using pure C.
...
A [video](https://www.youtube.com/watch?v=D7Sd8A6_fYU), where author tries to describe motivation of pure C heretics.
