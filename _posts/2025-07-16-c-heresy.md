...
[Hare](https://harelang.org) programming language is also heretic, by truing to be just a better C.
...
And it really adds many interesting imporvements and it even has some sort of templates (parametric generics).
...
* Some beginner programmers may get scared of C++ complexity and they reject it, because they don't want to invest their time to learn C++ properly.
  ...
  Continue using simpler language will lead to lower overall programming performance and less code reliability.
...
* Some game developers are also vulnerable to pure C heresy.
  ...
  But it's no longer the case, most popular modern consoles has C++ compilers available, so that game engines written in C++ (like Unreal Engine) support consoles.
...
The main advantage of C++ is that in places, where squeezing every bit of performance isn't necessary, one can use RAII ideom (like standard library containers), which is generally easier compared to manage each allocation individually, what is typical for C programs.

There is one more performance benefit of C++, compared to pure C, which is not obvious.
